...
4. Ejecuta **todas** las migraciones en Supabase, en orden (`001` a `011` — ver paso 1.3 más abajo), antes de usar la app: varios módulos dependen de que las anteriores ya hayan corrido (usuarios/roles, la regla de asignación automática, servicios, etapas del pipeline actual, catálogo de motivos de pérdida, formulario de ejemplo, políticas RLS por rol, tabla de buzones de correo).
