...
- `TRANSACTIONAL_EMAIL_PROVIDER` (`postmark`, `sendgrid` o `brevo`), `TRANSACTIONAL_EMAIL_API_KEY`, `TRANSACTIONAL_EMAIL_FROM` — sin esto, `notifyBayron.ts` simplemente no envía el correo de aviso a Bayron por lead nuevo ni el de actividad asignada (`notifyActivityAssigned`, ver README "Calendario y actividades"), y ambos flujos siguen funcionando igual (el aviso real de leads ya queda en el CRM vía `alertas`).
...
1. Crear cuenta en Postmark/SendGrid/Brevo.
