...
// POST /api/activity-update  { activity_id, estado?, resultado?, proxima_accion? }
// Used by the Completar/Reabrir toggle (estado only) and available for a
// future "agregar resultado" flow. Ownership: Administrador general, the
// lead's responsable, or the activity's own responsable — covers both "I
// own this lead" and "this task was assigned to me" cases.
export const handler: Handler = async (event) => {
  ...
  const { data: actividad, error: actividadError } = await admin
    .from("actividades")
    .select("id, estado, resultado, proxima_accion, responsable_id, lead:lead_id ( responsable_id )")
